...
def showAllInfo(tree, table):
    ...
    db = pymysql.connect(host="127.0.0.1", user="root",
                         password="123456", db="my_db_01", port=3306)
    cur = db.cursor()
...
def appendInfo(tree, table, list):
    ...
    db = pymysql.connect(host="127.0.0.1", user="root", password="123456", db="my_db_01", port=3306)
    cur = db.cursor()
...
def table1():  # 教职工基本信息表（teacher_basic）
    ...
    root1.title('基本信息表')
    Label(root1, text='基本信息表', bg='white', fg='red', font=('宋体', 15)).pack(side=TOP, fill='x')
...
def table2():  # 教职工教学信息表（teaching）
    ...
    root2.title('教学信息表')
    Label(root2, text='教学信息表', bg='white', fg='red', font=('宋体', 15)).pack(side=TOP, fill='x')
...
def table3():  # 教职工科研信息表（research）
    ...
    root3.title('科研信息表')
    Label(root3, text='科研信息表', bg='white', fg='red', font=('宋体', 15)).pack(side=TOP, fill='x')
...
def init():
    ...
    root0.title('教师信息管理系统')
    Label(root0, text='教师信息管理', bg='white', fg='red', font=('宋体', 15)).pack(side=TOP, fill='x')

    Button(root0, text="基本信息表", command=table1).place(relx=0.5, rely=0.2, width=100, anchor=CENTER)
    Button(root0, text="教学信息表", command=table2).place(relx=0.5, rely=0.4, width=100, anchor=CENTER)
    Button(root0, text="科研信息表", command=table3).place(relx=0.5, rely=0.6, width=100, anchor=CENTER)
...
Label(root, text='教师信息管理系统', bg='white', fg='red', font=('宋体', 15)).pack(side=TOP, fill='x')
